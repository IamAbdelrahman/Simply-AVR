...
void APP_Start(void) {
    vuint8_t i = 0;
    for (i = 0; i < 10; i++) {
        SevenSegment_DisplayDigit(PORTA, i);
        _delay_ms(500);
    }
    SevenSegment_Clear(PORTA);
    _delay_ms(1000);
    
    vuint8_t Letters[] = "ABCEFHLOPSU";
    vuint8_t size = sizeof(Letters) / sizeof(Letters[0]);
    for (i = 0; i < size; i++) {
        SevenSegment_DisplayLetter(PORTA, Letters[i]);
        _delay_ms(500);
    }
    SevenSegment_Clear(PORTA);
    _delay_ms(1000);
